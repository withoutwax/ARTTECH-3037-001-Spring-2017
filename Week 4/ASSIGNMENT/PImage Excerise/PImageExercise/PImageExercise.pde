...
PImage[] greyScale, maskImage;


void setup() {
  size(340, 120);
  
  greyScale = new PImage[3];
  
  greyScale[0] = loadImage("getThumbnail.jpeg");
  greyScale[1] = loadImage("bridgesFigure.png");
  greyScale[2] = loadImage("retroBG.png");
...
void draw() {
  ...
  for (int i = 0; i < macaron.length; i++) {
    image(macaron[i], xPosition[i], 10);
  }
