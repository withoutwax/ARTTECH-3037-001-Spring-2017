class Tetris {
  ...
  void render() {
    ...
    for (int i = 0; i < pile.size(); i++) {
      rect(pile.get(i).x*scl, pile.get(i).y*scl, scl, scl);
    }
  ...
  void update() {
    ...
    shapes.add(new PVector(shapeLoc.x, shapeLoc.y));
    ...
    shapes.get(currentShape).y = shapes.get(currentShape).y + (shapeSpeed * yDir);
    shapes.get(currentShape).x = shapes.get(currentShape).x + (shapeSpeed * xDir);

    shapes.get(currentShape).y = constrain(shapes.get(currentShape).y, 0, (boardHeight)); 
    shapes.get(currentShape).x = constrain(shapes.get(currentShape).x, 8, 22);
    ...
    if (shapes.get(currentShape).y > 0) {
      ...
      for (int i = 0; i < tetrisShapes.fallingShapes.size(); i++){       //for each box in the game board
        if (tetrisShapes.fallingShapes.get(i).y == (boardHeight-1)) {
          hit = true;
      ...
      for (int i = 0; i < tetrisShapes.fallingShapes.size(); i++) {
        if (tetrisShapes.fallingShapes.get(i).x == pile.get(i).x && tetrisShapes.fallingShapes.get(i).y == (pile.get(i).y-1)) {
        hit = true;
          pile.add(new PVector(tetrisShapes.fallingShapes.get(i).x, tetrisShapes.fallingShapes.get(i).y));
    ...
    //if (shapes.get(currentShape).y == boardHeight-(1*)) {
